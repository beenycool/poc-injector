...
struct JvmDll {
    ...
    uintptr_t base = 0;        // runtime base address of jvm library
    bool      ok   = false;

#ifdef __APPLE__
    // macOS: use dlopen/dlsym to find libjvm.dylib
    void* dl_handle = nullptr;

    bool init() {
        // Try to find the already-loaded JVM library
        // RTLD_NOLOAD: don't load, just find if already mapped
        dl_handle = dlopen("libjvm.dylib", RTLD_NOLOAD | RTLD_LAZY);
        if (!dl_handle) {
            // Try alternate names / paths
            dl_handle = dlopen("libjvm.so", RTLD_NOLOAD | RTLD_LAZY);
        }
        if (!dl_handle) {
            // Try searching via RTLD_DEFAULT (all loaded images)
            void* sym = dlsym(RTLD_DEFAULT, "JNI_GetCreatedJavaVMs");
            if (sym) {
                JNI_GetCreatedJavaVMs = (decltype(JNI_GetCreatedJavaVMs))sym;
                ok = true;
                return true;
            }
            return false;
        }
        base = (uintptr_t)dl_handle;

        void* p = dlsym(dl_handle, "JNI_GetCreatedJavaVMs");
        if (!p) return false;
...
// ---------------------------------------------------------------------------
// JVMTI types and constants
// ---------------------------------------------------------------------------

// JVMTI version constants
#define JVMTI_VERSION_1_0  0x30010000
#define JVMTI_VERSION_1_1  0x30010000
#define JVMTI_VERSION_1_2  0x30010002

// Opaque JVMTI environment pointer type
struct _jvmtiEnv;
typedef _jvmtiEnv* jvmtiEnv;

// --- JVMTI function pointer typedefs (we only need these three) ---
typedef jint (JNICALL *jvmti_GetLoadedClasses_t)(jvmtiEnv, jint*, jclass**);
typedef jint (JNICALL *jvmti_GetClassSignature_t)(jvmtiEnv, jclass, char**, char**);
typedef jint (JNICALL *jvmti_Deallocate_t)(jvmtiEnv, unsigned char*);

// JVMTI function table (first 4 entries only — enough for our needs)
struct jvmtiInterface_1_0_ {
    void*                              reserved0;       // 0x00
    jvmti_GetLoadedClasses_t           GetLoadedClasses; // 0x08
    jvmti_GetClassSignature_t          GetClassSignature;// 0x10
    jvmti_Deallocate_t                 Deallocate;       // 0x18
};

// jvmtiEnv wraps a pointer to the function table
struct _jvmtiEnv {
    const jvmtiInterface_1_0_*         functions;       // 0x00
};
