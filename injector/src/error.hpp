...
struct posix_error {
    ...
    posix_error(int c, const char* fn) : code(c), function_name(fn) {}

    std::string format() const {
        return function_name + " failed (errno="
               + std::to_string(code) + "): "
               + strerror(code);
    }
};

inline std::string format_last_error(const char* function_name) {
    return posix_error(errno, function_name).format();
}

inline void log_error(const char* function_name) {
    auto msg = format_last_error(function_name);
